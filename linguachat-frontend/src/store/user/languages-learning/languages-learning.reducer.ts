...
export const blockedUsersReducer = createReducer(
  ...
  on(LanguagesLearningActions.getResponseForLanguagesLearning, (state, { languages }) => {
    return langaugesLearningAdapter.setAll(languages, state);
  }),
  on(LanguagesLearningActions.getResponseToAddLanguageLearning, (state, { language }) => {
    return langaugesLearningAdapter.addOne(language, state);
  }),
  on(LanguagesLearningActions.getResponseToDeleteLanguageLearning, (state, { id }) => {
