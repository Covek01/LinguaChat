...
export class ConnectionController {
  ...
  async addConnection(
    ...
    return await this.connectionService
      .insertConnection(parseInt(firstUserId, 0), parseInt(secondUserId, 0))
      .catch((error) => {
  ...
  async deleteConnection(@Param('id') id: string): Promise<number> {
    return await this.connectionService
      .deleteConnection(parseInt(id, 0))
      .catch((error) => {
  ...
  async deleteConnectionBetwenUsers(
    ...
    return await this.connectionService
      .deleteConnectionBetweenUsers(parseInt(firstId, 0), parseInt(secondId, 0))
      .catch((error) => {
        console.log('Error with deleting connection');
        console.log(error);
  ...
  async getConnectionsOfUser(
    @Param('id') id: string,
  ): Promise<ConnectionGetDto[]> {
    return await this.connectionService
      .getConnectionsOfUser(parseInt(id, 0))
      .catch((error) => {
